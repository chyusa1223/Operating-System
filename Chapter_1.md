...
*   가상화는 운영체제가 다른 운영체제 내에서 하나의 응용처럼 수행될 수 있게 한다. 언뜻 보아서는 그러한 기능이 별 이유가 없는 것처럼 보인다. 그러나 가상화 산업은 방대하며 성장하고 있어 그 용도와 중요성을 증가하고 있다. 

*   넓게 말하면 가상화는 에뮬레이션을 포함하는 소프트웨어 집단의 한 구성원이다. 에물레이션은 소스 CPU 유형이 목표 CPU 유형과 다를 때 사용된다. 예를 들면 애플사가 자신의 데스크톱과 랩톱 컴퓨터의 CPU를 IBM Power CPU에서 인텔 X86 CPU로 전환하였을 때 "로제타"라는 에뮬레이션 설비를 포함하여 IBM CPU용으로 컴파일 되었던 응용을 인텔 CPU에서 수행할 수 있게 하였다. 이러한 개념이 한 기계를 위해 작성된 전체 운영체제를 다른 기계에서 수행되도록 확장될 수 있다. 그렇지만 에뮬레이션은 비용이 크다. 소스 시스템에서 수행되는 기계어 수준의 모든 명령은 목표 시스템에서 동등한 기능으로 번역되어야 하고 종종 이는 다수의 목표 명령어가 되기도 한다. 만일 소스와 목표 CPU가 비슷한 성능 수준일 경우 에뮬레이트된 코드는 원래의 코드보다 훨씬 느리게 수행된다. 에뮬레이션의 일반적인 예는 컴퓨터 언어가 기계어 코드가 아니라 고급 수준 형태로 수행되거나 또는 중간 형태로 번역되었을 경우 발생한다. 이는 인터프리테이션이라고 지칭한다. 베이직과 같음 몇몇 언어는 컴파일 또는 인터프리트 된다. 반면 자바는 항상 인터프리트된다. 인터프리테이션은 고급 언어 코드가 고유의 CPU 명령으로 번역된다는 점에서 에뮬레이션의 한 형태로 다른 CPU를 에뮬레이트하는 것이 그 언어가 수행될 수 있는 이론적인 가상 기계를 에뮬레이트 한다. 따라서 우리는 자바 프로그램을 자바 가상 기계에서 수행할 수 있지만 기술적으로 이들 가상기계는 자바 에뮬레이터이다. 

*   이와 달리 가상화에서는 특정 CPU를 위해 컴파일된 운영체제가 동일 CPU 용의 다른 운영체제 내에서 수행된다. 가상화는 다수 사용자가 작업을 병행 수행하기 위한 방법으로 IBM 대형 컴퓨터에 처음 등장하였다. 다수의 가상기계를 수행함으로써 많은 사용자들이 단일 사용자를 위해 설계된 시스템에서 작업을 수행할 수 있게 되었다. 후에 다수의 마이크로소프트 Windows XP 응용을 인텔 x86 CPU에 수행하는 문제를 해결하기 위해 VMWare가 XP상에서 수행되는 응용의 형태로 새로운 가상화 기술을 만들었다. 이 응용은 게스트(Guest)라 불리는 하나 이상의 Windows나 다른 x86용 운영체제 복사본들을 수행하며 각 게스트는 자신의 응용을 수행한다. 윈도우는 호스트 운영체제이고 VMWare 응용은 VMM이다. VMM은 게스트 운영체제를 수행하고 그들의 자원 이용을 관리하며 각 게스트를 서로로부터 보호한다. 

*   현대의 운영체제가 다수의 응용을 신뢰성 있게 수행할 능력이 있다고 해도, 가상화의 이용은 지속적으로 성장하고 있다. 랩톱과 데스크톱에서 VMM은 체험을 위하거나 또는 다른 호스트용으로 작성된 운영체제 상의 응용을 수행하기 위해 다수의 운영체제를 설치할 수 있게한다. 예를 들면 X86 CPU 위에서 Mac OS를 수행하는 애플 랩톱은 윈도우 응용을 수행하기 위해 윈도우 게스트를 수행할 수 있다. 다수 운영체제를 위해 소프트웨어를 작성하는 회사는 가상화를 이용하여 하나의 물리 서버를 이용하여 이들 모든 운영체제를 수행하여 이들 모든 운영체제를 수행하여 개발, 테스팅, 디버깅을 수행할 수 있다. 데이터 센터에서는 가상화가 계산 환경을 수행하고 관리하는 데 일반적이 되었다. VMWare ESX와 Citrix XenServer와 같은 VMM은 호스트 운영체제에서 수행된다기 보다 호스트 운영체제라 하는 것이 더 적절할 것이다. 
![VMWare](https://www.cs.uic.edu/~jbell/CourseNotes/OperatingSystems/images/Chapter1/1_20_VMware.jpg)      
...
*   내장형 시스템은 현재 가장 유행하는 컴퓨터의 형태이다. 이 장치들은 자동차 엔진, 공장용 로봇에서 VCR, 전자파 오븐 등 어느 곳에서나 볼 수 있다. 이 장치들은 아주 특정한 작업만을 수행하는 경향이 있다. 이 장치들이 수행되는 시스템은 매우 원시적이며 따라서 운영체제도 제한된 기능만을 제공한다. 일반적으로 사용자 인터페이스가 거의 없으며 자동차 엔진이나 로봇 팔과 같은 하드웨어 장치들을 감시하고 관리하는 데 시간을 보낸다. 

*   이 내장형 시스템들은 매우 다양하다. 어떤 시스템은 UNIX와 같은 범용 운영체제를 수행시키면서 특수 목적을 가진 응용프로그램을 수행시키는 형태를 갖는다. 다른 시스템은 필요한 기능만을 제공하는 내장형 운영체제를 가지고 있는 하드웨어의 형태를 취한다. 또 다른 시스템은 운영체제 없이 필요한 작업을 수행하는 응용 전용 칩(ASICs)만을 갖는 하드웨어 장치들이다. 

*   내장형 시스템의 사용은 점점 확산되고 있으며 독립적으로 동작하건 네트워크와 웹의 구성원으로 동작하건 이 장치들의 계산 능력 또한 증가 추세에 있다. 지금도 한 가정 전체가 컴퓨터화 될 수 있다. 이러한 가정에서는 중앙 컴퓨터(범용 컴퓨터나 내장형 컴퓨터)가 난방, 조명, 비상경보 시스템 심지어 커피 제조기 등을 관리한다. 웹을 통하여 집주인은 집에 도착하기 전에 난방 시스템을 가동시킬 수 있다. 언젠가는 냉장고 우유가 없는 것을 발견하고 가게에 전화를 거는 날이 올것이다. 

*   내장형 시스템은 거의 언제나 실시간 운영체제를 수행한다. 실시간 시스템은 처리기의 작동이나 데이터의 흐름에 엄격한 시간 제약이 있을 때 사용된다. 따라서 실시간 시스템은 종종 전용 응용에서 제어 장치로 사용된다. 감지기가 데이터를 컴퓨터로 가져온다. 컴퓨터는 반드시 데이터를 분석하고, 아마도 감지기의 입력을 변경할 수 있도록 제어를 조정할 것이다. 과학 실험을 제어하는 시스템, 의학 영상 시스템, 산업 제어 시스템, 그리고 몇몇 디스플레이 시스템 등이 실시간 시스템이다. 또한 일부 자동차 엔진 연료 분사 시스템, 가전 기기 제어기, 그리고 무기 시스템들도 실시간 시스템에 포함된다. 

*   실시간 시스템은 잘 정의된, 고정된 시간 제약을 가진다. 정의된 제한 시간 내에 처리가 반드시 이루어져어ㅑ 하며, 그렇지 않을 경우 시스템은 실패하게 된다. 예를 들면 로봇 팔이 제작중인 자동차를 파괴하고 난 후에 중지하라는 명령을 내린다면 아무 소용이 없을 것이다. 실시간 시스템은 정해진 시간 제약 내에 올바른 결과를 돌려줄 수 있을 때에만 정확히 동작하는 것으로 간주된다. 이러한 요구 조건을 빠른 응답이 바람직한 시분할 시스템이나, 전혀 시간적 제한을 받지 않는 일괄처리 시스템과 대조해 보라. 


## 12. 오픈소스 운영체제 

*   운영체제의 학습에 있어서 많은 수의 오픈소스 공개판의 가용성의 중요도는 이루 말할 수 없다. 오픈소스 운영체제는 컴파일된 이진 형태보다는 소스 코드 형태로 받을 수 있는 운영체제를 의미한다. Linux는 가장 유명한 오픈소스 운영ㅇ체제이며, 반면에 MS사의 Windows는 비공개소스 방식의 잘 알려진 예이다. 애플의 Mac OS X와 IOS 운영체제는 혼합형 접근을 취한다. 이들은 Darwin이라는 오픈 소스 커널을 포함하며 사유의 비공개 소스 요소도 포함하고 있다. 

*  소스 코드로부터 프로그래머는 한 시스템에서 실행 가능한 이진코드를 생성할 수 있다. 역공학(Reverse Engineering)을 통하여 이진코드로부터 소스 코드를 얻어내는 반대의 작업은 매우 많은 노력을 필요로 하며 주석과 같은 유용한 사항들을 복원해 낼 수는 없다. 소스코드를 분석하여 운영체제를 배우는 것은 매우 유용할 수 있다. 소스 코드를 가지고 있으므로 학생들은 운영체제를 수정하고 컴파일 하여 코드를 실행하면서 변경된 사항들을 시험해 볼 수 있으며, 이는 하나의 매우 훌륭한 학습 도구이다. 이 책은 중요한 모든 운영체제 주제들을 다룰 수 있으록 상위 수준에서 알고리즘을 설명할 뿐아니라, 운영체제 소스코드를 수정하는 것을 포함한 프로젝트도 제공하고 있다. 책 전반에 걸쳐 심화 학습을 위한 오픈 소스코드의 예를 가리키고 있다. 

*   오픈소스 운영체제는 많은 장점을 가지고 있다. 그 예로 관심 있는 프로그래머들의 공동체를 들 수 있으며, 이 프로그래머들은 디버깅에 도움을 주고, 코드를 분석하고, 지원을 제공하고, 수정 사항을 제안함으로써 코드 발전에 기여한다. 이론의 여지는 있지만, 더 많은 프로그래머들이 코드를 검토하기 때문에 오픈소스 코드가 비공개소스 코드에 비해 더 보안성이 높다. 확실히 오픈소스 코드는 버그를 포함하고 있지만 오픈소스 옹호자들은 코드를 사용하고 검토하는 사람들의 수 때문에 이 버그들이 빠르게 발견되고 수정되는 경향이 있다고 주장한다. 프로그램을 판매함으로써 이익을 얻는 회사들은 그들의 소스 코드를 공개하는데 주저하는 경향이 있지만, Red Hat 및 다른 매우 많은 회사들은 코드를 공개하고 있으며 상업적인 회사들도 소스 코드를 공개했을 때 손해 보는 것이 아니라 이익을 낼  수 있음을 증명하고 있다. 예를 들면 소프트웨어 지원 계약과 소프트웨어가 실행되는 하드웨어를 판매함으로써 이익을 창출할 수 있다. 

### 12.1 역사

*   현대 컴퓨팅 시대의 초창기에는 많은 수의 소프트웨어가 오픈소스 형식이었다. MIT 대학의 Tech Model Railroad Club의 원 해커들은 자신들의 프로그램을 다른 사람들이 작업할 수 있도록 서랍에 넣어 두었다. "자생적인" 사용자 그룹은 그들의 모임에서 소스 코드를 교환하였다. 후에 Digital Equipment Corporation의 DEC 같은 특정회사-관련 사용자 그룹은 소스 코드 프로그램의 기여를 받아들였으며, 이들을 모아 테이프로 만들어 관심 있는 구성원들에게 배포하였다. 

*   컴퓨터와 소프트웨어 회사들은 결국 인증된 컴퓨터나 돈을 지불한 사용자들만이 그들의 소프트웨어를 사용할 수 이ㅣㅆ도록 제한하는 방안을 찾게 되었다. 소스 코드 자체가 아니라 소스 코드로부터 컴파일 된 이진 파일을 배포함으로써 이러한 목적을 이루는 데 도움이 되었을 뿐 아니라 경쟁회사로부터 자신들의 소스 코드와 아이디어를 보호할 수 있게 되었다. 또 다른 이슈는 저작권 보호물이다. 운영체제와 다른 프로그램들은 인증된 컴퓨터에서만 영화와 음악을 재생할 수 있거나 전자책을 볼 수 있도록 제한 할 수 있다. 이런 복사 방지 또는 Digital Rights Management(DRM)은 이러한 제한을 구현한 소스코드를 공개하면 효과가 없을 것이다. U.S. Digital Milennium Copyright Act(DMCA)를 포함한 많은 나라의 법률은 DRM 코드를 역공학하거나 복사 방지를 우회하려는 시도를 불법으로 규정한다. 

*   소프트웨어 사용을 제한하려는 행보에 대한 반발로 Richard Stallman은 1983년에 무료 오픈소스 UNIX-호환 운영체제를 만들기 위한 GNU 프로젝트를 시작하였다. Stallman은 1985년에 모든 소프트웨어는 무료이어야 하고 소스 코드가 공개되어야 한다고 주장하는 GNU Manifesto를 발표하였다. 그는 또 소프트웨어 소스 코드의 무료 교환과 그 소프트웨어의 무료 사용 권장을 목적으로 하는 Free Software Foundation(FSF)를 창설하였다. FSF는 소프트웨어의 지적재산권을 보호하기 보다는 소프트웨어의 공유와 개선을 독려하는 카피레프트를 주장한다. GNU General Public Licence(GPL)은 카피레프팅을 체계적으로 명문화하고 소프트웨어가 공개되는 공통 라이선스다. 근본적으로 GPL은 반드시 소스 코드는 이진 파일과 함께 배포되어야 하고 그 소스 코드의 수정본 또한 동일한 GPL 라이선스 하에 배포될 것을 요구한다. 
 
### 12.2 Linux
*   오픈소스 운영체제의 예로서 GNU/Linux를 살펴보자. GNU 프로젝트는 컴파일러, 문서편집기 및 유틸리티를 포함한 많은 UNIX-호환 툴들을 제작하였으나, 커널을 배포하지는 않았다. 1991년 핀란드의 Linux Torvalds라는 학생이 GNU 컴파일러와 도구들을 이용하여 기본적으로 UNIX와 유사한 커널을 발표하고, 범세계적인 기여를 요청하였다. 인터넷의 보급은 관심 있는 누구나가 소스코드를 다운로드 받고, 수정하여 Torvalds에게 제출하는 작업을 가능하게 하였다. 일주일마다 수정 사항을 발표함으로써 소위 Linux라 불리는 이 운영체제가 급성장하고 수천 명의 프로그래머들에 의해 개선되는 결과를 낳았다. 

*   결과적으로 GNU/Linux 운영체제는 수백 개의 고유한 배포한 또는 맞춤형 시스템을 양산하였다. 주요 배포판으로는 RedHat, SUSE, Fedora, Debian, Slackware 및 Ubuntu 등이 있다. 배포판은 기능, 유틸리티, 설치 응용 프로그램, 하드웨어 지원, 사용자 인터페이스와 목적 면에서 다양한 차이를 보인다. 예를 들어 RedHat Enterprise Linux는 대규모 상업용 사용에 초점이 맞추어졌다. PCLinuxOS는 시스템의 하드 디스크에 설치 되지 않고 CD-ROM에서 부팅되고 실행되는 운영체제 형태인 LiveCD이다. PCLinuxOS의 한 변형인 "PCLinuxOS Supergamer DVD"는 그래픽 드라이버와 게임을 포함한 LiveDVD이다. 게이머는 DVD에서 부팅시킴으로써 호환되는 어느 컴퓨터에서든지 운영체제를 실행할 수 있다. 게이머가 작업을 종료한 후, 시스템을 재부팅하면 설치된 운영체제로 되돌아가게 된다. 

### 12.3 BSD Unix

*   BSD UNIX는 Linux보다 오래되고 복잡한 역사를 가지고 있다. 1978년에 AT&T의 파생물로 출발하였다. University of California at Berkeley(UCB)의 배포판은 소스와 이진 형태로 공개하였으나 AT&T로부터 라이선스를 얻어야 했기 때문에 오픈소스 버전은 아니었다. BSD UNIX의 발전은 AT&T에서 제기한 소송에 의해 더디어졌으나, 결국 완전히 가동 가능한 오픈소스 버전이 4.4BSD-lite가 1994년에 발표되었다. 

*   Linux와 마찬가지로 FreeBSD, NetBSD, OpenBSD 및 DragonflyBSD 등의 여러 배포판이 존재한다. FreeBSD의 소스 코드를 탐구하고 싶으면, 단지 위에서 설명한 Linux의 경우와 같이 원하는 버전의 가상 기계 이미지를 다운로드 받아 VMware에서 부팅시키면 된다. 소스 코드는 배포판에 포함되어 있으며, 위치는 /usr/src에 위치한다. 커널 소스코드는 /usr/src/sys에 존재한다. 예를 들면 FreeVSD 커널의 가상 메모리 구현을 탐구하기 위해서는 /usr/src/sys/vm에 존재하는 파일들을 살펴보면 된다.

*   MAC의 핵심 커널 구성요소인 Darwin은 BSD UNIX에 기반을 두고 있으며 역시 오픈소스화 되어 있다. 이 소스 코드는 http://www.opensource.apple.com/darwin-source/에서 다운로드 받을 수 있다. 모든 MAC OS X 배포판은 이 사이트에 오픈소스 구성요소가 발표된다. 커널을 포함하고 있는 패키지의 이름은 "xnu"이다. 또한 Aplle은 http://connnect.apple.com을 통하여 광범위한 개발 도구들, 문서 및 지원을 제공한다. 자세한 사항에 대해서는 부록 A를 참조하기 바란다. 

### 12.4 Solaris

*   Solaris는 Sun Microsystems사의 Unix 기반 상용 운영체제이다. 원래 SUn사의 SunOS 운영체제는 BSD UNIX에 바탕을 두었다. Sun은 1991년에 AT&T사의 System V UNIX로 기반을 변경하였다. 2005년에 Solaris 코드의 대부분을 OpenSolaris 프로젝트로 오픈소스화 하였다. Oracle사가 Sun을 2009년에 사들임에 따라 이 프로젝트의 상태가 북확실하게 되었다. 불행하게도 코드의 일부분이 아직 AT&T나 다른 회사들의 소유로 남아 있기 때문에 모든 Solaris 코드가 오픈소스화 된 것은 아니다. 그러나 http://opensolaris.org/os/downloads/에서 2005년 당시의 소스 코드를 다운로드 받아 소스 코드 브라우저로 볼 수 있다. 

*   OpenSolaris를 사용하는 데 관심을 가진 다수의 그룹이 그 기반에서 출발하여 기능을 확장하였다. 그들의 작업 집합은 Project Illumos로서 OJpenSolaris 기반에서 발전하여 다수의 추가 기능을 포함하고 있고 다수의 프로젝트의 기반이 되었다. Illumos는 http://wili.illumos.org에서 이용 가능하다. 

### 12.5 학습 도구로서 오픈 소스 시스템
*   무료 소프트웨어 운동은 운영체제를 포함하여 수천의 오픈 소스 프로젝트를 만드는 동인이 되고 있다. http://freshmeat.net와 http://distrowatch.com/ 같은 사이트는 다수의 이러한 프로젝트에 대해 포탈을 제공한다. 앞서 말한대로 오픈소스 프로젝트는 학생들이 소스 코드를 학습 도구로 사용할 수 있게 한다. 학생들은 프로그램을 변경하고 테스트하여 버그를 고치거나 또는 원숙한 완전한 기능의 운영체제나 컴파일러, 사용자 인터페이스 및 다른 프로그램들을 공부할 수 있다. Multics와 같은 역사적 프로젝트의 소스 코드가 사용 가능해져 학생들은 이들 프로젝트를 이해하고 새로운 프로젝트를 구현하는데 필요한 지식을 구축할 수 있다. 

*   GNU/Linux와 BSD UNIX는 모두 오픈소스 운영체제지만 각각의 목표, 유틸리티, 라이센싱 및 목적을 갖는다. 때로는 라이선스가 상호 배타적이지 않아 상호교배가 발생하여 운영체제 프로젝트의 급속한 개선을 낳기도 한다. 예를 들면 OpenSolaris의 다수 주요 구성요소가 BSD UNIX로 이식되었다. 무료 소프트웨어와 오픈 소싱의 이점이  오픈 소스 프로젝트의 수와 질을 향상시켜 이들 프로젝트를 사용하는 개인과 회사의 수를 증가시킬 것이다.  
